...
if (!em && sv && !pw) {
    ...
    var dEmail = GM_getValue('email', 'your.name@cdk.com'), //tampermonkey func(variableName, defaultValue)
        ...
        keyOptions = {
            ...
            key_5: 'A.d78638331b63ece0ee419964818f8e8d',
            key_6: 'A.50f3e84b941c37c0abf2132f3b989196',
        };
    // ----------------------------------------
    // advanced settings
    ...
    jQuery.each(keyOptions, function (val, text) {
        $keyOptions.append(jQuery('<option></option>').val(text).html(val));
    });
    // build input panel
    //    jQuery($wptInput)
    ...
    $wpt_butt.click(function () {
        ...
        var browser = jQuery('#bSelect option:selected').val(), // get value from drop down
            selectedKey = jQuery('#keySelect option:selected').val(), // get value from drop down
